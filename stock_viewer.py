...
st.caption("综合评分 = 技术面(40分) + 基本面(30分) + 资金面(30分)")
...
def _try_fetch_with_sina(symbol, lookback_days):
    try:
        df = ak.stock_zh_a_daily(
            symbol=f"sh{symbol}" if symbol.startswith('6') else f"sz{symbol}", adjust="qfq"
        )
        if df is None or df.empty:
            return None
        df["date"] = pd.to_datetime(df["date"])
...
if selected_date:
    ...
    if df_results.empty:
        ...
        # 先基于原始数据生成操作建议（避免 fillna 后 score 变成字符串）
        df_results['操作建议'] = df_results.apply(generate_advice, axis=1)
